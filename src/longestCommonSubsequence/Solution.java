...
public class Solution {
     ...
     * -1 这两个子串的最长公共序列 不选x，不选y，则dp[i][j] = dp[i-1][j-1]
     * -2 这两个子串的最长公共序列 选x，不选y，则dp[i][j] = dp[i][j-1]
     * -3 这两个子串的最长公共序列 不选x，选y，则dp[i][j] = dp[i-1][j]
     * -4 这两个子串的最长公共序列 选x，选y，则dp[i][j] = dp[i-1][j-1] + 1
     *
     * 注意 dp[i−1][j] 只是表示「必然不包含x，但可能包含y」的情况，也就是说 dp[i−1][j] 其实是该情况与情况 1 的合集。
     * 但是由于我们求的是「最大值」，只需要确保「不漏」即可保证答案的正确（某些情况被重复参与比较不影响正确性），因此这里直接使用 dp[i−1][j] 表示情况3没有问题。
     * 关于这部分的讲解看这个题解：https://leetcode.cn/problems/uncrossed-lines/solutions/788174/gong-shui-san-xie-noxiang-xin-ke-xue-xi-bkaas
     *
     * 这四种可能什么时候该考虑哪些呢？
